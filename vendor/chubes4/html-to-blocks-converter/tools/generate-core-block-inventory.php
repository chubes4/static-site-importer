...
if ( ! function_exists( 'html_to_blocks_generate_core_block_inventory' ) ) {
	/**
	...
	function html_to_blocks_generate_core_block_inventory( string $blocks_dir ): array {
		...
		foreach ( $files as $file ) {
			...
			$blocks[ $data['name'] ] = [
				...
				'attributes'      => array_values( array_keys( (array) ( $data['attributes'] ?? [] ) ) ),
				'supports'        => array_values( array_keys( (array) ( $data['supports'] ?? [] ) ) ),
				'allowedBlocks'   => $data['allowedBlocks'] ?? [],
				'parent'          => $data['parent'] ?? [],
				'ancestor'        => $data['ancestor'] ?? [],
				'usesContext'     => $data['usesContext'] ?? [],
				'providesContext' => array_values( array_keys( (array) ( $data['providesContext'] ?? [] ) ) ),
				'selectors'       => array_values( array_keys( (array) ( $data['selectors'] ?? [] ) ) ),
			];
		}
