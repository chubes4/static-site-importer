 ...
 */
namespace BlockFormatBridge\Vendor\Nette\Schema\Elements;

use BlockFormatBridge\Vendor\Nette;
use BlockFormatBridge\Vendor\Nette\Schema\Context;
use BlockFormatBridge\Vendor\Nette\Schema\Helpers;
        ...
        if (is_array($value) && is_array($base)) {
            ...
            foreach ($value as $key => $val) {
                if ($key === $index) {
                    ...
                    $base[$key] = array_key_exists($key, $base) && ($itemSchema = $this->items[$key] ?? $this->otherItems) ? $itemSchema->merge($val, $base[$key]) : $val;
                }
            }
            return $base;
        }
