 ...
 */
namespace BlockFormatBridge\Vendor\League\CommonMark\Util;

use BlockFormatBridge\Vendor\League\CommonMark\Exception\IOException;
/**
...
final class SpecReader
{
    ...
    public static function read(string $data): iterable
    {
        ...
        foreach ($matches as $match) {
            ...
            yield \trim($currentSection . ' #' . $exampleNumber) => ['input' => \str_replace('→', "\t", $match[2]), 'output' => \str_replace('→', "\t", $match[3]), 'type' => $match[1], 'section' => $currentSection, 'number' => $exampleNumber++];
        }
    }
    /**
     * @return iterable<string, array{input: string, output: string, type: string, section: string, number: int}>
